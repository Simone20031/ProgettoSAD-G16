...
public class LibreriaView implements Initializable, LibreriaObserver {
    ...
    // Riferimenti ai TextField attivi nel Dialog corrente (per evidenziazione
    // errori)
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        songListView.setPlaceholder(new Label("La libreria musicale è vuota.\nAggiungi un brano per iniziare!"));
        playlistListView.setPlaceholder(new Label("Nessuna playlist presente."));
        ...
        songListView.setCellFactory(lv -> {
            ListCell<String> cell = new ListCell<>() {
                ...
                {
                    btnOpzioni.setStyle(
                            "-fx-background-color: transparent; -fx-cursor: hand; -fx-font-weight: bold; -fx-padding: 0 4 0 4;");
                    ...
                    btnOpzioni.setOnAction(e -> {
                        ...
                        Stato stato = (currentPlaylist == null) ? new StatoLibreria()
                                : new StatoPlaylist(currentPlaylist);
    ...
    private void playSelected() {
        ...
        if (sel == null) {
            showAlert("Seleziona prima un brano.", Alert.AlertType.WARNING);
            return;
        }
        ...
        if (!Files.exists(file)) {
            showAlert("File non trovato: " + file, Alert.AlertType.ERROR);
            return;
        }
    ...
    private void refreshList() {
        ...
        List<IBrano> braniDaMostrare = (currentPlaylist != null) ? currentPlaylist.getBrani()
                : libreriaController.getBrani();
    ...
    private void addSong() {
        ...
        MetadataService.estraiMetadati(f, v -> {
            for (int i = 0; i < Math.min(v.length, values.length); i++)
                values[i] = v[i];
        });
        ...
        TextField tFilename = g.addRow("Filename:", f.getName());
        ...
        if (d.showAndWait().orElse(ButtonType.CANCEL) == ButtonType.OK) {
            try {
                int annoParsed = parseIntField(currentYearField.getText(), "anno",
                        "L'anno deve essere un numero intero.");
                int durataParsed = parseIntField(currentDurationField.getText(), "durata",
                        "La durata deve essere in secondi numerici.");
                ...
                mostraErrore(new ValidazioneException("Errore importazione: " + ex.getMessage()));
            }
        }
    }
    ...
    private void editSelected() {
        ...
        SongMetadata m = metadataMap.get(fn);
        if (m == null)
            m = new SongMetadata(fn, MetadataService.stripExtension(fn), "", "", "");
        ...
        if (d.showAndWait().orElse(ButtonType.CANCEL) == ButtonType.OK) {
            try {
                ...
                if (branoTarget == null) {
                    mostraErrore(new ValidazioneException("Brano non trovato in memoria."));
                    return;
                }
                ...
                mostraErrore(new ValidazioneException("Errore aggiornamento: " + ex.getMessage()));
            }
        }
    }
    ...
    private void handleAggiungiBranoAllaPlaylist() {
        ...
        if (disponibili.isEmpty()) {
            mostraErrore(new ValidazioneException("Tutti i brani sono già presenti in questa playlist.",
        ...
        for (IBrano ib : disponibili) {
            if (ib instanceof Brano brano) {
                ...
                displayNames.add(
                        (brano.getTitolo() != null && !brano.getTitolo().isBlank()) ? brano.getTitolo() + " — " + fn
                                : fn);
            }
        }
    ...
    private void resetBordiCampi() {
        ...
        if (currentDurationField != null)
            currentDurationField.setStyle(s);
    ...
    /**
     * Estrae il filename dall'elemento visualizzato nella lista (formato "Titolo —
     * filename").
     */
